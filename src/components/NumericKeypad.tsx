...
interface NumericKeypadProps {
  ...
  canSubmit: boolean;
  showHint?: boolean;
  /** Reset minimized state when this key changes (use round index) */
  roundKey?: number;
...
export default function NumericKeypad({
  ...
  return (
    <div
      className={`relative flex min-h-0 min-w-0 ${isMobileLandscape ? "w-[16.25rem]" : "w-[12.5rem] md:w-[13.75rem]"} shrink-0 flex-col self-start rounded-xl transition-[padding,gap] duration-300 ease-in-out ${minimized ? "px-1.5 py-1 gap-0" : "p-1.5 gap-1"}`}
      style={{
    ...
    >
      {/* Display */}
      <div
        className="relative rounded-lg px-3.5 flex shrink-0 items-center justify-end overflow-visible cursor-pointer transition-[height] duration-300 ease-in-out h-14 md:h-12"
        ...
        style={{
          ...
          border: minimized && !showHint ? "none" : showHint ? "none" : "2px solid rgba(56,189,248,0.28)",
          color: "#67e8f9",
          textShadow: showHint ? "none" : "0 0 12px rgba(103,232,249,0.85), 0 0 26px rgba(56,189,248,0.4)",
          letterSpacing: "0.08em",
        }}
      >
        {display}

        {/* Hint hand on display */}
        {showHint && (
          <div
          ...
          >
            <svg
              viewBox="0 0 80 100"
              width={isCoarsePointer ? 60 : 90}
              height={isCoarsePointer ? 72 : 108}
              overflow="visible"
            ...
            >
              <path
                ...
                fill="#67e8f9" stroke="rgba(2,6,23,0.98)" strokeWidth="4" strokeLinejoin="round" paintOrder="stroke"
              />
              <path
